...
import { addToStore, getStoredApp } from '../InstalledApps/installedApp';
    ...
    return (
        <div className='p-20 bg-[#D2D2D250]'>
            <div className="flex gap-10 border-b-1 pb-10 border-gray-300">
                <div>
                    <img className='h-[350px] w-[350px] object-cover rounded-xl' src={appImage} alt={singleData.title} />
                </div>
                <div>
                    <h1 className='text-4xl font-bold'>{singleData.title}</h1>
                    <p className='text-xl text-gray-400 font-bold mt-2'>Developed by <span className='text-violet-500'>{singleData.companyName}</span></p>
                    ...
                    <button onClick={() => { handleInstall(singleData.id); }} className='btn px-5 py-[22px] bg-green-400 text-white mt-8 text-xl' disabled={installed}>
                        {installed ? 'Installed' : `Install Now (${singleData.size}MB)`}
                    </button>
                </div>
            </div>
            <div className="border-b-1 py-10 pb-16 border-gray-300 h-[550px]">
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart layout="vertical" data={[...singleData.ratings].reverse()} margin={{ top: 20, right: 30, left: 40, bottom: 20 }}>
